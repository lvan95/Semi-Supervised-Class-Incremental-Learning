    ...
    def _update_representation(self, labeled_trainloader, unlabeled_trainloader, test_loader, optimizer, scheduler,ema_model):
        ...
        for epoch in range(160):
            ...
            for batch_idx in range(110):
                ...
                logits = de_interleave(logits, 2 * 7 + 1)
                aux_logits = de_interleave(aux_logits, 2 * 7 + 1)
                logits_x = logits[:batch_size]
                ...
                aux_logits_x = aux_logits[:batch_size]
                aux_logits_u_w, aux_logits_u_s = aux_logits[batch_size:].chunk(2)
                del logits, aux_logits
                ...
                loss_aux = F.cross_entropy(aux_logits_x, aux_targets)
                pseudo_label1 = torch.softmax(aux_logits_u_w.detach() / 1, dim=-1)
                max_probs1, targets_u1= torch.max(pseudo_label1, dim=-1)

                mask1 = max_probs1.ge(0.95).float()
                loss_aux1 = (F.cross_entropy(aux_logits_u_s, targets_u1,
                                          reduction='none') * mask1).mean()
                aux = loss_aux+ loss_aux1
                loss = Lx_clf + Lu_clf + aux
                loss.backward()
                losses.update(loss.item())
                ...
                p_bar.update()
            p_bar.close()
            #test_loss, test_acc = self._test(test_loader, self._network)
