...
# from utils.data_manager import accuracy
EPSILON = 1e-8
batch_size = 64
...
class BaseLearner(object):
    def __init__(self, args):
        ...
        self.output_dim = 128
        self.distill_proj_hidden_dim = 2048
    ...
    @property
    def samples_per_class_unlabled(self):
        if self._fixed_memory:
            return self._memory_per_class
        else:
            assert self._total_classes != 0, 'Total classes is 0'
            return (2000 // self._total_classes)
    @property
    ...
    def build_rehearsal_memory_unlabel(self,data_manager,per_class):
        if self._fixed_memory:
            self._construct_exemplar_unified(data_manager, per_class)
        else:
            self._reduce_exemplar(data_manager, per_class)
