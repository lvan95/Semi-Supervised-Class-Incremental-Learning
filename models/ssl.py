        ...
        for epoch in range(250):
            ...
            for batch_idx in range(80):
                ...
                Lu_clf = (F.cross_entropy(logits_u_s[:,self._known_classes:],targets_u,
                                       reduction='none') * mask).mean()
                pseudo_label1 = torch.softmax(logits_old_u_w[:,:self._known_classes].detach()/1, dim=-1)
                pseudo_label2 = torch.softmax(logits_old_u_s[:, :self._known_classes].detach() / 1, dim=-1)
                max_probs1, targets_u1 = torch.max(pseudo_label1, dim=-1)
                max_probs2, targets_u2 = torch.max(pseudo_label2, dim=-1)

                pseudo_label3 = torch.softmax(logits_old_x[:,:self._known_classes].detach()/1, dim=-1)
                max_probs3, targets_u3 = torch.max(pseudo_label3, dim=-1)
                Lu1_clf = F.cross_entropy(logits_u_w[:, :self._known_classes], targets_u1, reduction='none')
                Lu2_clf  = F.cross_entropy(logits_u_s[:, :self._known_classes], targets_u2, reduction='none')
                Lu3_clf =  F.cross_entropy(logits_x[:, :self._known_classes], targets_u3, reduction = 'none')

                loss = Lx_clf + Lu_clf + Lu1_clf +Lu2_clf + Lu3_clf
                loss.backward()
                losses.update(loss.item())
                losses_x.update(Lx_clf.item())
                losses_u.update(Lu_clf.item())
                losses_u1.update(Lu1_clf.item())
                ...
                mask_probs.update(mask.mean().item())
                p_bar.set_description(
                    "Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. LR: {lr:.4f}. Data: {data:.3f}s. Batch: {bt:.3f}s. Loss: {loss:.4f}. Loss_x: {loss_x:.4f}. Loss_u: {loss_u:.4f}. Loss_u1:{loss_u1:.4f}. Mask: {mask:.2f}. ".format(
                        epoch=epoch + 1,
                        epochs=250,
                        batch=batch_idx + 1,
                        iter=80,
